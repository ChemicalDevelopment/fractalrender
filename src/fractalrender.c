...
int main(int argc, char *argv[]) {
    ...
    // the reason this is STR instead of FLOAT is to support multiprecision
    cargs_add_arg("-c", "--center", 2, CARGS_ARG_TYPE_STR, "");
    cargs_add_default_i("-c", "0.0", 0);
    cargs_add_default_i("-c", "0.0", 1);

    cargs_add_arg("-z", "--zoom", 1, CARGS_ARG_TYPE_STR, "");
